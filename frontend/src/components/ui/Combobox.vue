...
const anchorClass = tw`typo-label2-regular flex min-h-48 w-full cursor-pointer border-2 border-blue-800 bg-white py-4 pr-4 pl-16 -outline-offset-4 outline-blue-800 placeholder:text-gray-800 hover:outline-4 has-focus-visible:outline-4`;

const inputClass = tw`w-full bg-transparent focus-visible:outline-hidden`;

const buttonClass = tw`flex cursor-pointer items-center p-8 text-blue-800 hover:bg-blue-100 hover:text-blue-800 focus-visible:bg-blue-800 focus-visible:text-white focus-visible:outline-none`;

const triggerActiveClass = tw`bg-blue-800 text-white`;
...
const contentClass = tw`pointer-events-auto z-20 max-h-[var(--reka-combobox-content-available-height)] w-[var(--reka-combobox-trigger-width)] overflow-auto bg-white px-8 py-12 shadow-md`;

const itemClass = tw`flex min-h-48 cursor-pointer flex-col justify-center gap-2 border-l-4 border-transparent px-12 py-10 data-[highlighted]:-ml-4 data-[highlighted]:border-blue-600 data-[highlighted]:bg-blue-200 data-[state=checked]:-ml-4 data-[state=checked]:border-blue-800 data-[state=checked]:bg-blue-200`;
</script>
...
<template>
  ...
  >
    <ComboboxAnchor :class="anchorClass">
      ...
      <ProgressSpinner
        v-if="loading"
        class="!h-24 !w-24 shrink-0 !text-current"
      />
