...
import de.bund.digitalservice.ris.search.SharedTestConstants;
...
import de.bund.digitalservice.ris.search.testData.CaseLawTestData;
import de.bund.digitalservice.ris.search.testData.NormsTestData;
import de.bund.digitalservice.ris.search.testData.SortingTestArguments;
...
class AllDocumentsSearchControllerAPITest extends ContainersIntegrationBase {
  ...
  void shouldReturnCorrectResultForSearchWithDateFilter() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.member[0]['item'].decisionDate", is(SharedTestConstants.DATE_2_1.toString())))
        .andExpect(
            jsonPath(
                "$.member[1]['item'].legislationDate",
                is(SharedTestConstants.DATE_2_1.toString())));
  }
  ...
  void shouldReturnCorrectResultForSearchWithDateLtFilter() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.member[0]['item'].legislationDate", is(SharedTestConstants.DATE_1_1.toString())))
        .andExpect(
            jsonPath(
                "$.member[0].textMatches[?(@.name == 'name')].text",
                Matchers.containsInAnyOrder(is("<mark>Test</mark> Gesetz Nr. 2"))));
  }
  ...
  void testSpecialCharacters(String searchTerm, String expectedDocumentNumber) throws Exception {
    ...
    mockMvc
        .perform(get(ApiConfig.Paths.DOCUMENT + query).contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.member[0].item.documentNumber", is(expectedDocumentNumber)));
  }
  ...
  void testForStopWords() throws Exception {
    mockMvc
        ...
        .andExpectAll(
            ...
            jsonPath("$.member[0].item.documentNumber", is("BFRE000107055")));
  }
  ...
  void testForExceededPageSize() throws Exception {
    mockMvc
        ...
        .andExpectAll(
            status().is(422),
            jsonPath("$.errors[0].code", is("invalid_parameter_value")),
            jsonPath("$.errors[0].parameter", is("size")),
            jsonPath("$.errors[0].message", is("size must not exceed 100")));
  }
  ...
  void testForZeroPageSize() throws Exception {
    mockMvc
        ...
        .andExpectAll(
            ...
            jsonPath("$.errors[0].code", is("invalid_parameter_value")),
            jsonPath("$.errors[0].parameter", is("size")),
            jsonPath("$.errors[0].message", is("size must be at least 1")));
  }
}
