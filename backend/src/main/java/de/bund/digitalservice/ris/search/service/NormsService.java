  ...
  public Page<Norm> getWorkExpressions(WorkEli workEli, Pageable pageable) {
    SearchSourceBuilder sourceBuilder =
        new SearchSourceBuilder()
            .query(QueryBuilders.termQuery("work_eli.keyword", workEli.toString()))
            .sort("entry_into_force_date", SortOrder.DESC)
            .docValueField("expression_eli.keyword")
            .docValueField("entry_into_force_date", "yyyy-MM-dd")
            .docValueField("expiry_date", "yyyy-MM-dd")
            .fetchSource(false)
    ...
    try {
      ...
      var norms =
          Arrays.stream(response.getHits().getHits())
              .map(
                  hit -> {
                    String expressionEli = hit.getFields().get("expression_eli.keyword").getValue();
                    LocalDate entryIntoForceDate =
                        hit.getFields().get("entry_into_force_date") != null
                            ? LocalDate.parse(
                                hit.getFields().get("entry_into_force_date").getValue())
                            : null;
                    LocalDate expiryDate =
                        hit.getFields().get("expiry_date") != null
                            ? LocalDate.parse(hit.getFields().get("expiry_date").getValue())
                            : null;
