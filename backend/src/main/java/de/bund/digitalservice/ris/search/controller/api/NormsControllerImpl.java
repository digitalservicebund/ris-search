...
@Tag(
    ...
    description =
        ...
        """)
public class NormsControllerImpl implements NormsController {
   ...
   *
   * @param norm the object to uniquely identify a norm expression
  ...
  public ResponseEntity<LegislationExpressionSchema> getLegislation(NormExpressionId norm) {

    Optional<Norm> result = normsService.getByExpressionEli(norm.toExpressionEli());
   ...
   *
   * @param normWorkId the parts of the path used to identify a norm work
   * @param pagination the pagination parameters defining page size and index
  ...
  public CollectionSchema<LegislationExpressionSearchSchema> getWorkExamples(
      NormWorkId normWorkId, PaginationParams pagination) {
    Page<Norm> expressions =
        normsService.getWorkExpressions(
            normWorkId.toWorkEli(),
            PageRequest.of(pagination.getPageIndex(), pagination.getSize()));
