 ...
 */
@Component
@Profile({"dev", "e2e", "staging", "uat", "test", "prototype", "production"})
public class CaseLawBucket extends ObjectStorage {

  public CaseLawBucket(
      @Qualifier("caseLawS3Client") ObjectStorageClient s3Client,
      @Value("${s3.file-storage.case-law.versionPrefix}") String versionPrefix) {
    super(s3Client, LogManager.getLogger(CaseLawBucket.class), versionPrefix);
  }
}
