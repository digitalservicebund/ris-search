...
public class SitemapsUpdateJob implements Job {
  ...
  private final Integer SITEMAP_URLS = 40000;
  ...
  public void createSitemapsForNorms() {
    ...
    List<List<ExpressionEli>> batches =
        ListUtils.partition(expressions.stream().toList(), SITEMAP_URLS);
  ...
  public void createSitemapsForCaselaw() {
    List<String> paths =
        caseLawBucket.getAllKeys().stream()
            ...
            .toList();
    List<List<String>> batches = ListUtils.partition(paths.stream().toList(), SITEMAP_URLS);
    for (int i = 0; i < batches.size(); i++) {
