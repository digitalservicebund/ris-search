...
public class SitemapsUpdateJob implements Job {
  ...
  public void createSitemapsForNorms() {
    ...
    List<List<ExpressionEli>> batches = ListUtils.partition(expressions.stream().toList(), 50000);
  ...
  public void createSitemapsForCaselaw() {
    List<String> paths =
        caseLawBucket.getAllKeys().stream()
            ...
            .toList();
    List<List<String>> batches = ListUtils.partition(paths.stream().toList(), 50000);
    for (int i = 0; i < batches.size(); i++) {
